...
def hello_world():
    ...
    for i in range (0, numRows):
        ...
        for i in range (0, 5):
            followedUser = twitterFollows[i].screen_name
            followedTwitterLink = str('https://twitter.com/' + followedUser)
            investorName = twitterUser.screen_name
            investorTwitterLink = str('https://twitter.com/' + investorName)

            investorHyperLink = f'=HYPERLINK(\"{investorTwitterLink}\",\"{investorName}\")'
            followedUserHyperlink = f'=HYPERLINK(\"{followedTwitterLink}\",\"{followedUser}\")'

            wks.append_row([thisWeeksID, (str(twitterFollows[i].screen_name) + '_' + vcName), followedUserHyperlink, vcName, twitterFollows[i].description, investorHyperLink, twitterUser.description], value_input_option='USER_ENTERED')
        time.sleep(5)
